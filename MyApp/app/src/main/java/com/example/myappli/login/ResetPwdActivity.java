...
public class ResetPwdActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        EditText vcode = this.findViewById(R.id.v_codeEdit2);
        EditText newpwd = this.findViewById(R.id.passWordEdit);
        Button v_codeGetButton = this.findViewById(R.id.v_codeSendCommitButton2);
        Button ResetPwdCommitButton = this.findViewById(R.id.ResetCommitButton);
        ...
        v_codeGetButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (TextUtils.isEmpty(email.getText())) {
                    Toast.makeText(ResetPwdActivity.this, "请输入邮箱", Toast.LENGTH_SHORT).show();
                } else {
        ...
        ResetPwdCommitButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (TextUtils.isEmpty(userName.getText())) {
                    Toast.makeText(ResetPwdActivity.this, "请输入用户名", Toast.LENGTH_SHORT).show();
                } else if (TextUtils.isEmpty(email.getText())) {
                    Toast.makeText(ResetPwdActivity.this, "请输入邮箱", Toast.LENGTH_SHORT).show();
                }else if (TextUtils.isEmpty(vcode.getText())) {
                    Toast.makeText(ResetPwdActivity.this, "请输入邮箱验证码", Toast.LENGTH_SHORT).show();
                }else if (TextUtils.isEmpty(newpwd.getText())) {
                    Toast.makeText(ResetPwdActivity.this, "请输入新密码", Toast.LENGTH_SHORT).show();
                }
                else {
                    strUserName = userName.getText().toString();
                    strPassWord = newpwd.getText().toString();
                    strMail = email.getText().toString();
    ...
    private void okhttpData() {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                OkHttpClient client = new OkHttpClient();
                String jsonObject = String.format("{\"new_password\": \"%s\",\"username\": \"%s\",\"v_code\": \"%s\"}",strPassWord, strUserName,strVcode);
                RequestBody body = RequestBody.create(JSON, jsonObject);
                Request request = new Request.Builder()
                        .url("http://39.105.180.206:9000/reset_password")
                        .post(body)//Post请求的参数传递
                        .build();
                client.newCall(request).enqueue(new Callback() {
                    @Override
    ...
    private void jsonEmailData(String jsondata,Response response) {//解析JSON
        if (jsondata != null) {
            ...
            if (response.code() == 200) {
                ...
                Looper.prepare();
                Toast.makeText(ResetPwdActivity.this, "错误，请稍候", Toast.LENGTH_LONG).show();
                Looper.loop();
            }
        }
    }
    ...
    private void jsonJXData(String jsondata,Response response) {//解析JSON
        if (jsondata != null) {
            ...
            if (response.code() == 200) {
                Looper.prepare();
                Toast.makeText(this,"修改成功，请登录", Toast.LENGTH_LONG).show();
                Looper.loop();
            }else if(response.code() == 400){
                Looper.prepare();
                Toast.makeText(this,"用户不存在或验证码错误", Toast.LENGTH_LONG).show();
